...
def polar_to_cart(X_polar):
    ...
    X_cart_1 =[] 
    X_cart_2 =[]       
    for i in range(len(X_polar)):
        X_cart_1.append(np.sqrt(X_polar[i,0]**2+X_polar[i,1]**2))
        X_cart_2.append(2* np.arctan(X_polar[i,1]/(X_polar[i,0]+(np.sqrt(X_polar[i,0]**2+X_polar[i,1])))))
        
    X_cart = np.concatenate((X_cart_1,X_cart_2),axis=1)
